...
import { MOCK_PLANTS, MOCK_DEVICES } from '@/constants/garden'
import { LinearGradient } from 'expo-linear-gradient'
import BottomSheet, { BottomSheetBackdrop } from '@gorhom/bottom-sheet'
import { CancelConfirmModal, RuleSetupModal } from '@/components/iot'
import { updateAutomationRules, ZoneDetail } from '@/apis/zone.api'
...
export default function ZoneDetailScreen() {
  ...
  const activeRelays = linkedDevice?.hardware_config ? (Object.values(linkedDevice.hardware_config.relays).filter((v: any) => v !== 'null') as string[]) : [];
  const activeSensors = linkedDevice?.hardware_config ? Object.values(linkedDevice.hardware_config.sensors).filter(Boolean).length : 0;
    ...
    <View style={styles.root}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        <View style={styles.overviewContainer}>
          <Image source={{ uri: zone.image_url || 'https://images.unsplash.com/photo-1596434452758-52fb58fce47c?auto=format&fit=crop&q=80&w=800' }} style={styles.coverImage} />
        ...
        <View style={styles.body}>
          <View style={styles.section}>
            ...
            <LinkedPlantCard 
               ...
               onAddPlant={() => handleOpenAssign('plant')} 
            />
          </View>
